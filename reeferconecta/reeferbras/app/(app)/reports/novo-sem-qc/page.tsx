        ...
        <form className="mt-8 space-y-6" onSubmit={submit}>
          ...
          {forms.map((form, index) => (
            <section className="grid gap-5 rounded-xl border border-slate-700 p-4 sm:p-6" key={index}>
              ...
              <label className="grid gap-2 text-sm font-semibold text-slate-200">Situação<select className="rounded-lg border border-slate-300 px-3 py-2 font-normal text-white" value={form.situacaoAtual} onChange={(event) => update(index, "situacaoAtual", event.target.value)} required><option value="">Selecione</option>{situations.map((item) => <option key={item}>{item}</option>)}</select></label>
