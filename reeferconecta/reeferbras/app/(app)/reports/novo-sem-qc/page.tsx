    ...
    <main className="min-h-screen bg-gray-800 px-4 py-8 text-slate-900 sm:px-6 sm:py-10">
      <section className="mx-auto max-w-3xl">
        ...
        <form className="mt-8 space-y-6" onSubmit={submit}>
          ...
          <label> <button className="rounded-lg bg-red-700 px-4 py-3 font-semibold text-white disabled:opacity-50" disabled={saving} type="submit">{saving ? "Salvando..." : `Salvar ${forms.length} relatório(s)`}</button></label>
          {forms.map((form, index) => (
            ...
            <section className="grid gap-5 rounded-xl border border-slate-700 p-4 sm:p-6" key={index}>
              ...
              <label className="grid gap-2 text-sm font-semibold text-slate-200">Situação<select className="rounded-lg border border-slate-300 px-3 py-2 font-normal" value={form.situacaoAtual} onChange={(event) => update(index, "situacaoAtual", event.target.value)} required><option value="">Selecione</option>{situations.map((item) => <option className="text-black" key={item}>{item}</option>)}</select></label>
